...
public abstract class AbstractSqlEventDao implements SqlEventDao {
	...
	public List<Event> findByKeyword(Connection connection, String keywords,
		...
		Timestamp timeSt = datesSt != null ? new Timestamp(datesSt.getTime()
				.getTime()) : null;
		Timestamp timeEnd = datesEnd != null ? new Timestamp(datesEnd.getTime()
				.getTime()) : null;
		...
		try (PreparedStatement preparedStatement = connection
			...
			while (resultSet.next()) {
				...
				events.add(new Event(eventId, name, description, dateSt,
